...
fetch('http://localhost:3001/api/job/jobs/', {
  headers: {
    ...
    .then(jobs => {
      ...
      const filteredJobs = jobs.filter(job => job.jobTitle.toLowerCase().includes(keyword.toLowerCase()));
      ...
      filteredJobs.forEach(job => {
        ...
        locationCell.textContent = job.postedBy;
        row.appendChild(locationCell);
        const typeCell = document.createElement('td');
        const date1 =new Date(job.datePosted)
        typeCell.textContent = (date1).getDay() + "/" + ((date1).getMonth()+1) + "/" + (date1).getFullYear() +"";
        row.appendChild(typeCell);
        const companyCell = document.createElement('td');
        companyCell.textContent = job.jobDescription;
